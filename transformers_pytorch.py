...
class SelfAttention(nn.Module):
	...
	def forward(self, values, keys, queries, mask):
		N = queries.shape[0]
		value_len, key_len, query_len = values.shape[1], keys.values[1], queries.values[1]
	...
	keys = values.reshape(N, key_len, self.heads, self.heads_dim)
	...
	out = torch.einsum("nhql,nlhd->nqhd", [attention, values]).reshape(N, query_len, self.heads*se\
		.heads_dim)
	out = self.fc_out(out)
	return out
...
class TransformerBlock(nn.Module):
	def __init__(self, embed_size, heads, dropout, forward_expansion):
		...
		self.norm1 = nn.LayNorm(embed_size)
		self.norm2 = nn.LayNorm(embed_size)
		self.feed_forward = nn.Sequential(
			nn.Linear(embed_sizem, forward_expansion*embed_size),
			nn.RulU(),
...
class DecoderBlock(nn.Module):
	def __init__(self, embed_size, heads, forward_expansion, dropout, device):
		...
		self.attention = SelfAttention(embed_size, heads)
		self.norm = nn.LayNorm(embed_size)
		self.transformer_block = TransformerBlock(embed_size, heads, dropout, forward_expansion)
		self.dropout = nn.Dropout(dropout)

	def forward(self, x, values, keys, src_mask, trg_mask):
		attnetion = self.attention(x,x,x,trg_mask)
...
class Transformer(nn.Module):
	def __init__(
		...
		embed_size=256, num_layers=6, forward_expansion=4, heads=9,
...
if __name == "__main__":
	x = torch.tensor([[2,3,4], [4,4,5]])
	trg = torch.tensor([[2,5,2], [1,1,5]])
	src_pad_idx = 0
	trg_pad_idx = 1
	src_vocab_size = 3
	trg_vocab_size = 2
